package org.axgl.util.debug {
	...
	public class AxDebugger extends AxGroup {
		...
		public function AxDebugger() {			
			...
			libraryText = new AxText(4, 4, AxResource.font, Ax.LIBRARY_NAME + " @[160,160,160]Version @[150,150,255]" + version[0] + "@[10,255,255].@[180,180,255]" + version[1] + "@[255,255,255].@[210,210,255]" + version[2] + " " + debugMode);
			libraryText.scroll.x = libraryText.scroll.y = 0;
			libraryText.zooms = libraryText.countTris = false;
			this.add(libraryText);
			
			fpsText = new AxText(4, Ax.height - HEIGHT + 4, AxResource.font, "FPS: 0/0");
			fpsText.scroll.x = fpsText.scroll.y = 0;
			fpsText.zooms = fpsText.countTris = false;
			this.add(fpsText);
			
			memoryText = new AxText(70, Ax.height - HEIGHT + 4, AxResource.font, "Memory: 0MB");
			memoryText.scroll.x = memoryText.scroll.y = 0;
			memoryText.zooms = memoryText.countTris = false;
			this.add(memoryText);
			
			modeText = new AxText(0, 4, AxResource.font, "---", Ax.width - 3, "right");
			modeText.scroll.x = modeText.scroll.y = 0;
			modeText.zooms = modeText.countTris = false;
			this.add(modeText);
			
			timeText = new AxText(0, Ax.height - HEIGHT + 4, AxResource.font, "---", Ax.width - 5, "right");
			timeText.scroll.x = timeText.scroll.y = 0;
			timeText.zooms = timeText.countTris = false;
			this.add(timeText);
			
			titleText = new AxText(0, 4, AxResource.font, "", Ax.width, "center");
